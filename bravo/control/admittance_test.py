...
bravo_kin = kp.build_serial_chain_from_urdf(open("C:/Users/marcu/OneDrive/Documents/GitHub/bravo-admittance-control/bravo/config/urdf/bravo7.urdf").read(), "ee_link")
...
##### Selected Parameters #####
selection_mat = np.diag([0, 0, 0, 0, 0, 1]) # Selection matrix - ensures that the admittance controller only generates motion in the specified directions
Md = np.diag([0, 0, 0, 0, 0, 10])           # Mass matrix
Kp = np.diag([0, 0, 0, 0, 0, 10])           # Proportional gain
Kd = np.diag([0, 0, 0, 0, 0, 10])           # Derivative gain (not used in our formulation since we're doing parallel force/position)
Kf = np.diag([0, 0, 0, 0, 0, 1])            # Force gain
x_d = np.array([0, 0, 0, 0, 0, 0])          # Desired ee-position
f_d = np.array([0, 0, 0, 0, 0, 0.1])        # Desired ee-force

##### Sensor Readings #####
joint_configs = [0, pi / 2, pi, 0, pi, 0, 0, 0, 0]   # 6 body joints, 1 prismatic joint for gripper and 2 revolute joints for jaw joints
...
x_e_dot = np.array([0, 0, 0, 0, 0, 0.1])    # Current ee-velocity - Incoming sensor reading
...
joint_vel = np.linalg.pinv(jc) @ np.linalg.pinv(Md) @ (-Kd @ x_e_dot - Kp @ (x_d - x_e + Kf @ (f_d - selection_mat @ f_e)))
print(joint_vel)
